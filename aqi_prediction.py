...
class OptimizedAQIPredictor:
    ...
    def load_data(self, station_city=None):
        """Optimized data loading with cache clearing for city change"""
        cache_key = f"data_{station_city}"
        
        # Clear cache if city has changed
        ...
        try:
            ...
            if station_city:
                try:
                    ...
                    if not file_row.empty:
                        ...
                        logger.info(f"No data file for {station_city}, creating sample data")
                        return self.create_sample_data(station_city)
                except FileNotFoundError:
                    logger.info(f"Stations_Info.csv not found, creating sample data for {station_city}")
                    return self.create_sample_data(station_city)
            else:
                try:
                    ...
                    logger.info("data.csv not found, creating sample data")
            ...
            if missing_cols:
                logger.info(f"Missing columns: {missing_cols}, creating sample data")
                return self.create_sample_data(station_city)
            ...
            if self.df.empty:
                logger.info("Dataframe empty after cleaning, creating sample data")
                return self.create_sample_data(station_city)
    ...
    def calculate_aqi_from_pm25(self, pm25):
        """Accurate AQI calculation for PM2.5"""
        try:
            pm25 = float(pm25)
            if pm25 < 0:
                pm25 = 0
    ...
    def get_current_stats(self, data):
        """Accurate current stats calculation"""
        try:
    ...
    def get_hourly_distribution(self, data, pollutant):
        """Accurate hourly distribution calculation"""
        try:
            if not data:
    ...
    def get_simple_predictions(self, data, pollutant='PM2.5', n_future=24):
        """Improved prediction using weighted moving average"""
        ...
        try:
            ...
            # Get recent values (last 12 hours)
    ...
    def generate_simple_plots(self, data, predictions, city, pollutants=['PM2.5', 'PM10', 'SO2', 'NO2', 'CO', 'O3']):
        """Generate plots for all pollutants"""
        try:
            plot_data = {}
...
def cleanup_cache():
    ...
    try:
        ...
        logger.info(f"Cache cleanup completed. Removed {total_removed} entries")
    except Exception as e:
...
@app.route('/')
def index():
    """Optimized main route with all pollutants"""
    try:
        ...
        # Process data for all pollutants
        pollutants = ['PM2.5', 'PM10', 'SO2', 'NO2', 'CO', 'O3']
        current_stats, error = predictor.get_current_stats(filtered_data)
        if error:
            current_stats = {}

        pollutant_trends, error = predictor.get_pollutant_trends(filtered_data)
        if error:
            pollutant_trends = {}
        ...
        # Generate predictions and hourly distributions for all pollutants
        predictions = {}
        ...
        for pollutant in pollutants:
            ...
            preds, _, error = predictor.get_simple_predictions(filtered_data, pollutant)
            if error:
                preds = [25.0] * 24
            predictions[pollutant] = preds

        # Generate plots for all pollutants in overview and trends views
        plot_data = {}
        if view_type in ['overview', 'trends']:
            plot_data, error = predictor.generate_simple_plots(filtered_data, predictions, selected_city, pollutants)
            if error:
                logger.error(f"Plot generation error: {error}")
                plot_data = {}
...
def api_dashboard():
    ...
    try:
        ...
        logger.error(f"Error in api_dashboard route: {str(e)}")
        return jsonify({
            'error': 'API Error',
...
def export_predictions():
    ...
    try:
        ...
        predictions_str = request.args.get('predictions', '')
        
        if not predictions_str:
            return jsonify({'error': 'No predictions provided'}), 400
        
        try:
            predictions = [float(x.strip()) for x in predictions_str.split(',') if x.strip()]
        except:
            return jsonify({'error': 'Invalid predictions format'}), 400
        
        if not predictions:
            return jsonify({'error': 'No valid predictions found'}), 400
        ...
        logger.error(f"Error in export_predictions route: {str(e)}")
        return jsonify({
            'error': 'Export Error',
